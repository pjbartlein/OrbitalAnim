...
# output .pngs path
png_path <- paste(path_prefix, "Projects/Calendar/animations/data/orb_solstice_anm/", sep="")
...
# color numbers
mcv <- 255
solstice_anm_clr <- c("#CC3300", "#FF6633", "#FFCC00", "#FFFFCC", "#99FFFF", "#00CCFF", "#0000FF", "#000066", "#CCCCCC")
solstice_anm_breaks <- c(-400, -10.0, -5.0, -2.0, 0.0, 2.0, 5.0, 10.0, 400)
...
for (i in (1:nt)) {
  ...
  png_file <- paste(png_path, "orb_solstice_anm_", png_name, "_ka.png", sep="")
  ...
  for (m in (1:12)) {
    solstice_anm <- abs((monlen[i, (14 + m)] -  173.0)) - abs((monlen[151, (14 + m)]) -  173.0)
    colornum <- cut(solstice_anm, solstice_anm_breaks, right=TRUE)
    if (i == 151) colornum = 9
    ...
    if (idx_end_inner < idx_start_inner) {
      ...
      for (ii in (idx_start_inner:npts)) {
        ...
        segments(x_seg_inner, y_seg_inner, x_seg_outer, y_seg_outer, lwd=2, col=solstice_anm_clr[colornum])
      }
  ...
  # scale
  xorig <- -1.0; yorig <- -1.7; dx <- 0.2; dy <- 0.07
  xleft <- xorig + seq(0, dx*7, by=dx)
  xright <- xleft + dx
  ybottom <- rep(yorig, 8)
  ytop <- ybottom + dy
  rect(xleft, ybottom, xright, ytop, col=solstice_anm_clr, lwd=1.5)
  text(xleft[5], ytop + dy, "Mid-month Day - June Solstice Differences (days)")
  for (l in (1:7)) {
    text(xleft[1+l], ybottom - dy/2, solstice_anm_breaks[1+l], cex=0.8)
  }
  text(xleft[1], ybottom - dy/2, "(closer)", cex=0.8)
  text(xright[8], ybottom - dy/2, "(farther)", cex=0.8)
  
  dev.off()
  }
  
  # ImageMagick 7 command line
  # magick -delay 50 @orb_solstice_anm_files.txt -resize 1080x1080 -loop 0 -layers Optimize orb_solstice_anm.gif
