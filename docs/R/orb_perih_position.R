# orb_position.R
# orbital positions at 5-degree intervals 150 ka to 0 ka
...
# output .pngs path
png_path <- paste(path_prefix, "Projects/Calendar/animations/data/orb_perih_position/", sep="")
...
for (i in (1:nt)) {
  ...
  png_file <- paste(png_path, "orb_perih_position_", png_name, "_ka.png", sep="")
  ...
  # equal-step true anomalies -- for Earth position
  rp <- a * (1.0 - ecc^2) / (1.0 + ecc * cos(angle5))
  xp <- rp * cos(angle5) + 2 * ae
  yp <- rp * sin(angle5)
  rot_xy <- rotate(xp, yp, perih_angle ) #- radians(3))
  xp <- rot_xy[[1]]; yp <- rot_xy[[2]]
  p_angle <- pos_angle(degree(atan2(yp, xp) + pi))
  
  # perihelion x and y
  theta_perih <- pi 
  r_perih =  (a * (1.0 - ecc^2) / (1.0 + ecc * cos(theta_perih))) + 0.05
  ...
  # Earth
  points(xp, yp, pch=16, cex = 1.5, col = "lightblue")
  points(xp, yp, pch=1, cex = 1.5, lwd = 0.5, col = "darkblue")
  ...
  title <- paste(age, "ka")
  text((-1*lim)+.5, (-1*lim)+.1, title, cex=2, pos=2)
  ...
  peri_char <- paste("=", sprintf("%.2f", orb_params$Perih_deg[i]), "°")
  peri_title <- expression(omega)
  text(xpos, ypos-0.2, peri_title)
  text(xpos, ypos-0.2, peri_char, pos=4)
...
# magick -delay 50 @orb_position_files.txt -resize 1080x1080 -loop 0 -layers Optimize orb_position.gif
