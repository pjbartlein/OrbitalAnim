...
# output .pngs path
png_path <- paste(path_prefix, "Projects/Calendar/animations/data/orb_monangle/", sep="")
...
for (i in (1:nt)) {
  ...
  png_file <- paste(png_path, "orb_monangle_", png_name, "_ka.png", sep="")
  ...
  for (m in (1:12)) {
    ...
    start_angle <- (mon_beg_noleap[m] * (360/365) + 180) 
    ...
    end_angle <- (mon_end_noleap[m] * (360/365) + 180) 
    ...
    mid_angle <- (mon_mid_noleap[m] * (360/365) + 180) 
    ...
    segments(0, 0, x_seg_inner, y_seg_inner, lwd=2, col="gray80")
  ...
  # magick -delay 50 @orb_monangle_files.txt -resize 1080x1080 -loop 0 -layers Optimize orb_monangle.gif
